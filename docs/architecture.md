...
[4] Cloudflare (DNS/CDN/WAF + Tunnel)
          ...
          ▼
[3] mgmt 베어메탈 (이 저장소)                        ← Rocky 9.8 / 20c / 62GB
     Docker Compose: edge·observability·security·cicd·data·backup
     k3d: 중앙 Argo CD 관리 클러스터
          ▲ 메트릭/로그 수집           ▲ 이미지 pull / GitOps
[2] 원격 Kubernetes (팀원별 master+worker1/2) ── App · CSI/CCM · 익스포터
          ▲ VM/네트워크/스토리지
[1] OpenStack AIO (Keystone·Nova·Neutron·Cinder·Glance·Octavia·...)
```

1·2·4 계층은 **연동 대상**이며 mgmt에서 직접 기동하지 않는다. 중앙 Argo CD만
mgmt의 k3d에서 실행하고 원격 Kubernetes API를 관리한다.

## 2. 오케스트레이션 모델

- 각 Docker 서비스 = **독립 Compose 프로젝트**
  (`compose/stacks/<도메인>/<서비스>/compose.yaml`).
- 중앙 Argo CD = `k3d/config.yaml`로 생성한 별도 Kubernetes 관리 클러스터.
...
- 운영 진입점은 루트 `Makefile`이다. Compose는 `make compose-up s=...`,
  k3d는 `make cluster-create`로 실행한다.
...
`node-exporter`·`kube-state-metrics`·`OpenCost` 는 K8s 측 배포 → mgmt Prometheus의
**스크레이프 타깃**으로 등재
(`compose/stacks/observability/prometheus/config/prometheus.yml`). 로그는
Beats/log DaemonSet → mgmt ELK. Velero는 K8s 배포이고 백업 타깃 버킷만
mgmt MinIO에 생성.
