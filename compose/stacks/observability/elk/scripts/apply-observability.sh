...
#
# 주의: 이 ELK 스택은 xpack.security 가 꺼져 있어 Space 는 "조직적 분리"이지
#       "접근 제어"가 아니다(누구나 Space 전환 가능). 진짜 격리는 xpack.security + role
#       활성화가 필요하며 이는 별도 작업이다.
...
curl -sf -X PUT "$ES/_ilm/policy/logs-retention-14d" \
...
curl -sf -X PUT "$ES/_index_template/acer-logs" \
...
curl -sf -X PUT "$ES/k8s-logs-*,infra-logs-*/_settings" \
...
KBH=(-H 'kbn-xsrf: true' -H 'Content-Type: application/json')
