...
for app in kibana prometheus alertmanager vault adguard traefik minio semaphore; do
  add_rewrite "${app}.teleport.${BASE_DOMAIN}"
done

# The former sibling tp-* names cannot work with Teleport's proxy-domain rule.
sed -i -E '/^[[:space:]]+- domain: tp-(grafana|kibana|prometheus|alertmanager|keycloak|vault|netbox)\./,+2d' "$CONFIG_FILE"
