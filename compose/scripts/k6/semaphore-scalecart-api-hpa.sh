...
for argument in "$@"; do
  case "$argument" in
    K6_RATE=*) export SCALECART_K6_RATE="${argument#K6_RATE=}" ;;
    K6_DURATION=*) export SCALECART_K6_HOLD_DURATION="${argument#K6_DURATION=}" ;;
...
case "${SCALECART_K6_RATE:-150}" in *[!0-9]*|'') echo "K6_RATE must be a positive integer" >&2; exit 2 ;; esac
case "${SCALECART_K6_HOLD_DURATION:-4m}" in *[!0-9smh]*|'') echo "K6_DURATION must use seconds, minutes, or hours" >&2; exit 2 ;; esac
