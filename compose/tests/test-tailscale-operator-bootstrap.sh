...
jq -e 'length == 5 and all(.[]; has("team") and has("project") and has("operator_vault_path") and has("bootstrap_vault_path") and has("argocd_vault_path") and has("proxy_mode") and has("proxy_hostname"))' "$manifest" >/dev/null || fail "invalid team bootstrap manifest"
jq -e '.[] | select(.team == "ggg" and .proxy_mode == "inprocess" and .proxy_hostname == "ggg-operator-0")' "$manifest" >/dev/null || fail "ggg must use the in-process ggg-operator-0 proxy"
...
grep -Fq 'if [[ "$proxy_mode" == inprocess ]]' "$task" || fail "in-process proxy mode is required"
grep -Fq 'operatorConfig.hostname="$proxy_hostname"' "$task" || fail "in-process hostname is required"
